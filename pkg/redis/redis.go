...
type (
	RedisService interface {
		...
		SetWithExpire(key string, value interface{}, expiresDuration time.Duration) error
	}
...
func (service *RedisImp) Delete(key string) error {
	ctx := context.Background()
	result, err := service.client.Del(ctx, key).Result()
	if err != nil {
		log.Printf("error cause: %+v\n", err)
		return err
	}
	fmt.Printf("total data deleted:%d\n", result)
	return nil
}

func (service *RedisImp) SetWithExpire(key string, value interface{}, expiresDuration time.Duration) error {
	ctx := context.Background()
	result, err := service.client.SetEX(ctx, key, value, expiresDuration).Result()
	if err != nil {
		log.Printf("error cause: %+v\n", err)
		return err
	}
